...
## Миграция 1: Добавление истории личных рекордов

✅ **Применено в основной схеме** 

Создаёт таблицу для отслеживания истории изменений личных рекордов пользователя. Позволяет видеть прогресс во времени и строить графики.

```sql
-- Создаём таблицу истории рекордов
...
-- Комментарии
COMMENT ON TABLE exercise_record_history IS 'История изменений личных рекордов для отслеживания прогресса';
COMMENT ON COLUMN exercise_record_history.achieved_at IS 'Дата и время достижения рекорда';
COMMENT ON COLUMN exercise_record_history.workout_session_id IS 'Ссылка на тренировку, где был установлен рекорд';
```

**Примеры использования:**

```sql
-- Добавить новый рекорд в историю
INSERT INTO exercise_record_history 
  (user_id, exercise_id, weight, reps, achieved_at, notes)
VALUES 
  ('user-uuid', 'exercise-uuid', 100.0, 5, NOW(), 'Новый PR! 🎉');
...
WHERE user_id = 'user-uuid' AND exercise_id = 'exercise-uuid'
ORDER BY achieved_at DESC;

-- График прогресса за последние 3 месяца
SELECT 
...
WHERE user_id = 'user-uuid' 
  ...
  AND achieved_at >= NOW() - interval '3 months'
...
## Миграция 3: Добавление поля для аватара упражнения
...
## Миграция 4: Добавление категорий упражнений
...
## Миграция 5: Добавление заметок к упражнениям в сессии
...
## Миграция 6: Добавление оценки сложности тренировки
...
## Миграция 7: Добавление таблицы для селфи после тренировки
...
## Миграция 8: Добавление целей (goals)
...
## Миграция 9: Добавление AI-подсказок
...
## Миграция 10: Добавление супер-сетов
...
## Миграция 11: Добавление публичных шаблонов
...
## Миграция 12: Добавление таблицы для трекинга веса тела
...
### Откат миграции 1 (история рекордов)

```sql
DROP TABLE IF EXISTS exercise_record_history CASCADE;
```

### Откат миграции 3 (аватары упражнений)
...
### Откат миграции 7 (фото прогресса)
